...
from schemas.user_schemas import UserCreate, UserResponse, UserWithToken, Token, UserUpdate
...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_db_session)):
    if await user_service.get_user_by_name(db, user.name):
        raise HTTPException(
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    response.set_cookie(key="refresh_token", value=refresh_token, httponly=True, max_age=max_age, samesite='None',
                        secure=True)
